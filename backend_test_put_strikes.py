...
class FCNPutStrikesTester:
    ...
    def test_analyze_fcn_with_put_strikes(self, symbols, fcn_params):
        """Test FCN analysis with put strike prices"""
        data = {
            "symbols": symbols,
    ...
    def print_fcn_analysis_results(self, analysis):
        ...
        # Print strike prices
        strike_prices = fcn_params.get('strike_prices', {})
        print("\nStrike Prices:")
        for symbol, price in strike_prices.items():
            print(f"  {symbol}: ${price:,.2f}")
        
        # Print put strike prices
        put_strike_prices = fcn_params.get('put_strike_prices', {})
        print("\nPut Strike Prices:")
        for symbol, price in put_strike_prices.items():
            print(f"  {symbol}: ${price:,.2f}")
        ...
        for symbol, metrics in fcn_metrics.items():
            ...
            print(f"    Strike Price: {currency_symbol}{metrics.get('strike_price', 0):,.2f}")
            print(f"    Put Strike Price: {currency_symbol}{metrics.get('put_strike_price', 0):,.2f}")
            ...
            moneyness = metrics.get('moneyness')
            if moneyness is not None:
                print(f"    Moneyness: {moneyness:,.2f}%")
            else:
                print(f"    Moneyness: N/A")
                
            put_moneyness = metrics.get('put_moneyness')
            if put_moneyness is not None:
                print(f"    Put Moneyness: {put_moneyness:,.2f}%")
            else:
                print(f"    Put Moneyness: N/A")
...
def main():
    ...
    # Test Case 1: Full FCN with Put Strikes
    print("\n🔍 Test Case 1: Full FCN with Put Strikes")
    ...
    success_us_basket, analysis_us_basket = tester.test_analyze_fcn_with_put_strikes(["AAPL", "MSFT"], fcn_params_us_basket)
    
    # Test Case 2: Mixed US/HK with Put Strikes
    print("\n🔍 Test Case 2: Mixed US/HK with Put Strikes")
    ...
    success_mixed_basket, analysis_mixed_basket = tester.test_analyze_fcn_with_put_strikes(["AAPL", "0700.HK"], fcn_params_mixed_basket)
